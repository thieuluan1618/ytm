...
class DislikeManager:
        ...
        try:
            if os.path.exists(self.dislikes_file):
                with open(self.dislikes_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    self._disliked_ids = set(song.get('videoId', '') for song in data.get('songs', []) if song.get('videoId'))
    ...
    def dislike_song(self, song: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            print("[yellow]This song will be skipped in future searches and radio playlists[/yellow]")
            return True
