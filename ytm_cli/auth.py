...
class AuthManager:
    ...
    def select_credential_file(self, credential_files: List[Tuple[str, Dict[str, str]]]) -> Optional[Dict[str, str]]:
    ...
    def setup_oauth_auth(self, client_id: str, client_secret: str, open_browser: bool = True) -> bool:
        ...
        try:
            ...
            if ('verification' in error_str and 'process' in error_str) or \
               ('access_denied' in error_str) or \
               ('not completed' in error_str and 'verification' in error_str):
    ...
    def setup_browser_auth_interactive(self, open_browser: bool = True) -> bool:
        ...
        choice = input("\nPress Enter when cURL is copied to clipboard (or 'q' to quit): ").strip().lower()
        if choice == 'q':
            return False
        ...
        while True:
            try:
                ...
                if line.startswith("-H '") and line.endswith("'"):
                    ...
                    print("[yellow]⚠️  Expected format: -H 'header-name: value' or 'header-name: value'[/yellow]")
    ...
    def _setup_browser_from_file_with_guidance(self) -> bool:
        ...
        try:
            ...
            if len(curl_content) > 1000:
                ...
                print(f"[yellow]⚠️  Small command ({len(curl_content)} characters) - verify it's complete[/yellow]")
            ...
            if success:
                ...
                response = input(f"\n[yellow]Delete {file_path} for security? (Y/n): [/yellow]").strip().lower()
                if response != 'n':
                    try:
                        os.remove(file_path)
    ...
    def get_ytmusic_instance(self) -> YTMusic:
        ...
        try:
            if method == 'oauth' and os.path.exists(self.oauth_file):
                ...
                print(f"[yellow]Auth method '{method}' configured but credentials not found, using unauthenticated[/yellow]")
                return YTMusic()
    ...
    def _parse_raw_headers(self, headers_raw: str) -> Dict[str, Any]:
        ...
        for line in lines:
            if ':' in line:
                ...
                if key.lower() in ['cookie', 'user-agent', 'x-goog-visitor-id', 
